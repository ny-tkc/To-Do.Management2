...
const NavItem = ({ page, activePage, setActivePage, icon, label }: NavItemProps) => {
  ...
  return (
    <button
      ...
      className={`flex-1 flex flex-col items-center justify-center pt-2 pb-2 transition-colors duration-200 min-w-0 ${
        isActive ? 'text-cyan-600' : 'text-slate-500 hover:text-cyan-500'
      }`}
    >
      <Icon name={icon} className={`mb-0.5 ${isActive ? 'text-cyan-600' : ''}`} size={20} />
...
export const BottomNav = ({ activePage, setActivePage }: BottomNavProps) => {
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] flex justify-around items-center z-50 pb-[env(safe-area-inset-bottom)] h-14">
